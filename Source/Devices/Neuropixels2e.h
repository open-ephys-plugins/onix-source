...
#ifndef __NEUROPIXELS2E_H__
#define __NEUROPIXELS2E_H__
...
public:
	...
	uint64_t getProbeSerialNumber(int index);

	static const int numberOfChannels = 384;

	static const int baseBitsPerChannel = 4;
	static const int configurationBitCount = numberOfChannels * baseBitsPerChannel / 2;
	...
	static const int registersPerShank = electrodesPerShank + referencePixelCount + dummyPixelCount;
	...
	ShankBitsArray static makeShankBits(NeuropixelsV2Reference reference, std::array<ElectrodeMetadata, numberOfElectrodes> channelMap);

	template<int N>
	void writeShiftRegister(uint32_t srAddress, std::bitset<N> bits);

	std::array<std::unique_ptr<ProbeSettings<numberOfChannels, numberOfElectrodes>>, 2> settings;
...
private:
	...
	void defineMetadata(ProbeSettings<numberOfChannels, numberOfElectrodes>*, int);

	int m_numProbes = 0;

	static const int numFrames = 10;

	float samples[numberOfChannels * numFrames];
