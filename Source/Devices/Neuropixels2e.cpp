...
Array<int> Neuropixels2e::selectElectrodeConfiguration(String config)
{
	Array<int> selection;
...
int Neuropixels2e::configureDevice()
{
	...
	for (int i = 0; i < m_numProbes; i++)
	{
		...
		settings[i] = std::make_unique<ProbeSettings<numberOfChannels, numberOfElectrodes>>();
		defineMetadata(settings[i].get(), 4);
	}
...
bool Neuropixels2e::updateSettings()
{
	for (int i = 0; i < 2; i++)
	{
		if (probeSN[i] != 0)
		{
			if (gainCorrectionFilePath[i] == "None")
			{
			...
			{
				LOGE("The gain correction file for probe ", probeSN[i], " does not exist.");
				return false;
			}
...
void Neuropixels2e::processFrames()
{
	while (!frameArray.isEmpty())
	{
		...
		uint16_t probeIndex = *(dataPtr + 4); //  TODO: CHECK IF THIS IS 0-INDEXED OR 1-INDEXED
		uint16_t* amplifierData = dataPtr + 6; // TODO: Double check this pointer reference index
		...
		for (int i = 0; i < FramesPerSuperFrame; i++)
		{
			...
			{
				samples[rawToChannel[j][i] + frameCount * numberOfChannels] =
		...
		if (shouldAddToBuffer)
		{
			...
			amplifierBuffer[probeIndex]->addToBuffer(samples, sampleNumbers, timestamps, eventCodes, numFrames);
		}
...
void Neuropixels2e::defineMetadata(ProbeSettings<numberOfChannels, numberOfElectrodes>* settings, int shankCount)
{
	...
	settings->probeMetadata.electrodes_per_shank = electrodesPerShank;
	settings->probeMetadata.rows_per_shank = electrodesPerShank / 2;
