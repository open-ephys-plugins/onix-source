    ...
    if (voltage != "Auto")
    {
        ...
        else
        {
            int result = AlertWindow::show (MessageBoxOptions()
                                                ...
                                                .withMessage (std::string ("There is currently a voltage override selected for this port. Would you like to keep this voltage [Keep Voltage] ") + "or allow the automated voltage discovery algorithm [Automated Discovery] to determine the best voltage for the new headstage selected?")
...
void OnixSourceEditor::refreshComboBoxSelection()
{
    ...
    if (resetPortA)
        headstageComboBoxA->setSelectedItemIndex (0, dontSendNotification);
    if (resetPortB)
        headstageComboBoxB->setSelectedItemIndex (0, dontSendNotification);
}
