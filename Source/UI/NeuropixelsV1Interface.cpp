    ...
    : SettingsInterface (d, e, c)
{
    ...
    if (device != nullptr)
    {
        ...
        infoLabel->setBounds (deviceEnableButton->getX(), deviceEnableButton->getBottom() + 10, deviceLabel->getWidth(), 80);
...
void NeuropixelsV1Interface::updateInfoString()
{
    ...
    if (device != nullptr)
    {
        ...
        infoString += npx->getProbePartNumber();
        infoString += "\n";
        infoString += "Flex: ";
        infoString += npx->getFlexPartNumber();
        infoString += "\n";
    }
...
void NeuropixelsV1Interface::buttonClicked (Button* button)
{
    ...
    else if (button == saveJsonButton.get())
    {
        ...
        if (fileChooser.browseForFileToSave (true))
        {
            ...
            try
            {
                ProbeInterfaceJson::writeProbeSettingsToJson (fileChooser.getResult(), npx->settings[0].get());
            }
            catch (const error_str& e)
            {
...
void NeuropixelsV1Interface::saveParameters (XmlElement* xml)
{
    ...
    xmlNode->setAttribute ("probePartNumber", npx->getProbePartNumber());
...
void NeuropixelsV1Interface::loadParameters (XmlElement* xml)
{
    ...
    for (auto* node : xml->getChildIterator())
    {
        ...
        {
            if (node->getStringAttribute ("name").toStdString() == npx->getName() && node->getIntAttribute ("idx") == npx->getDeviceIdx())
            {
                xmlNode = node;
                break;
            }
